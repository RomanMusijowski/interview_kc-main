...
services:
  ...
  kafka-ui:
    ...
    ports:
      - "8085:8080"
    environment:
      KAFKA_CLUSTERS_0_NAME: finance-cluster
  ...
  kafka-init:
    image: confluentinc/cp-kafka:7.5.0
    container_name: finance-kafka-init
    depends_on:
      kafka:
        condition: service_healthy
    environment:
      KAFKA_BOOTSTRAP_SERVERS: kafka:9093
      KAFKA_TOPIC_NAME: ${KAFKA_TOPIC_NAME:-user-transactions}
    entrypoint: ["/bin/sh", "-c"]
    command: |
      ...
      kafka-topics.sh --create \
        ...
        --topic ${KAFKA_TOPIC_NAME:-user-transactions} \
        --partitions 3 \
